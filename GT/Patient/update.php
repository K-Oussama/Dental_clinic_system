...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($nom_err) && empty($prenom_err) && empty($adresse_err)){
        // Prepare an update statement
        $sql = "UPDATE patient SET Nom=?, Prenom=?, tel=?, adresse=?, sexe=?, dob=?, description=?  WHERE ID=$id";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssss", $param_nom, $param_prenom, $param_tel, $param_adresse, $param_sexe, $param_dob, $param_description);
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <a href="/gestion%20dentaire/patient/patient.php" class="btn btn-secondary ml-2">Cancel</a>
                    </form>
                </div>
            </div>        
        </div>
